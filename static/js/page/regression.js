...
function showXTable(){
	...
	var tbstr='<table class="table table-striped table-hover table-sm"><tr><th colspan="4">自变量(X)列表</th></tr><tr><th>序号</th><th>名称</th><th>类型</th><th>移除</th></tr></thead><tbody>';
	for(var i=0;i<TAGS_SERIAL.length;i++){
		var key = TAGS_SERIAL[i].Id;
		tbstr +='<tr id="TagsXRow_'+key+'"><td>'+(i+1)+'</td><td>'+TAGS[key].Name+'</td><td>'+TAGS[key].TagType+'</td><td><div><button type="button" class="btn btn-outline-danger btn-sm" onclick="onRemoveXTag('+key+')" title="从列表中移除该变量">移除</button></div></td></tr>';
	}
	tbstr +='</tbody></table>';
...
function showYTable(){
	//console.log(TAGS_SERIAL);
	var tbstr='<table class="table table-striped table-hover table-sm"><tr><th colspan="4">因变量(Y)列表</th></tr><tr><th>名称</th><th>类型</th><th>移除</th></tr></thead><tbody>';
	for(let key in TAG_Y){
		tbstr +='<tr id="TagsYRow_'+TAG_Y[key].Id+'"><td>'+TAG_Y[key].Name+'</td><td>'+TAG_Y[key].TagType+'</td><td><div><button type="button" class="btn btn-outline-danger btn-sm" onclick="onRemoveYTag('+key+')" title="从列表中移除该变量">移除</button></div></td></tr>';
	}
	tbstr +='</tbody></table>';
...
function getRegressionRes(ajaxdata){//根据Ajax反馈的结果更新Tag的实时数据
	...
	//------------------------------------------------------------------
	var notable_r='<span class="badge badge-danger">不显著</span>';
	if (reg.R > reg.Ra){
		notable_r='<span class="badge badge-success">显著</span>';
	}
	var notable_t='<span class="badge badge-danger">不显著</span>';
	if (reg.T > reg.Ta){
		notable_t='<span class="badge badge-success">显著</span>';
	}
	var significance='<tr><td>复相关系数</td><td>'+reg.R.toFixed(dotp)+'</td><td>'+reg.Ra.toFixed(dotp)+'</td><td>'+notable_r+'</td></tr><tr><td>F值</td><td>'+reg.F.toFixed(dotp)+'</td><td>'+reg.Fa.toFixed(dotp)+'</td><td>'+notable_t+'</td></tr>';
	...
	ShowModal("分析结果",'<div class="alert alert-success"><strong>成功!</strong> 回归分析完成，请下滑鼠标查看结果。</div><div class="pull-right"><a class="btn btn-outline-success" role="button" href="#ViewRegResult">点我跳转</a></div>');
}
//=========读取请求区域============================================================
//读取变量快照请求
...
{
	if(groupnum<0){
		$("#LoadDataMsg").html('<div class="text-info">正在加载因变量Y的数据……</div>');
	}else{
		$("#LoadDataMsg").html('<div class="text-info">正在加载第['+(groupnum+1)+']个自变量X的数据……</div>');
	}
	//调用公用的loadXMLDoc函数
    loadXMLDoc(urlstr,function()
    {
        ...
        {//添加请求完成后的处理功能---------------------------------------
			...
			if(groupnum<0){
				$("#LoadDataMsg").html('<div class="text-success">因变量Y的数据加载完成</div>');
			}else{
				$("#LoadDataMsg").html('<div class="text-success">第['+(groupnum+1)+']个自变量X的数据加载完成</div>');
			}
			TIME_CHANGE = false;
        }//请求完成后的处理功能结束---------------------------------------
    });
...
{
	$("#LoadDataMsg").html('<div class="text-info">正在加载回归分析数据……</div>');	
	//调用公用的loadXMLDoc函数
    loadXMLDoc(urlstr,function()
    {
        if (xmlhttp.readyState==4 && xmlhttp.status==200)//请求处理完成，且状态OK
        {//添加请求完成后的处理功能---------------------------------------
			...
			$("#LoadDataMsg").html('<div class="text-success">回归分析数据加载完成</div>');
        }//请求完成后的处理功能结束---------------------------------------
    });
}
...
$(document).ready(function() {
	...
	$("#TagUnit").before("无")
	$("#TagTime").before("无")
	$("#AddTagToYTable").attr("title","选中有效变量后本按钮才可使用,只可选择一个因变量");
	$('#AddTagToYTable').html("选为因变量(Y)");
	$("#AddTagToXTable").attr("title","选中有效变量后本按钮才可使用");
	$('#AddTagToXTable').html("选为自变量(X)");
	$("#RemoveAll").attr("title","移除全部已选变量!");
	$('#RemoveAll').html("全部移除");
	$("#Submit").attr("title","将所选变量提交分析!");
	$('#Submit').html("提交分析");
	$("#SelectedAnalysisVariable").before("已选分析变量")
	$("#TagsTableSmallTitle").text('双击左侧树状结构中的变量标签或者选中标签后点击上面👆的按钮的方式将变量加入列表');
	$("#SelectedTags_y").before("因变量(Y)列表")
	$("#SelectedTags_x").before("自变量(X)列表")
	$("#HisSerialRemark").html('说明：当前所选变量的趋势图数据为原始历史数据,对比趋势图中的数据为等间隔聚合之后的数据');
	$("#result").before("回归分析结果")
	$("#test").before("回归系数及其检验")
	$("#regressionEquation").before("回归方程")
	$("#InTheFormula").before("式中：")
	$("#varianceAnalysis").before("方差分析")
	$("#RegressionSignificanceTest").before("回归显著性检验")
	$("#DataGraphicAnalysis").before("数据图形分析")
	$("#Echarts_Scatter").before("散点图区域")
	$("#Echarts_Trend").before("数据趋势图")
	$("#DataAnalysisTable").before("数据分析表")
