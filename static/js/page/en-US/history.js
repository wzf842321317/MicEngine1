...
function clearView(){
	HIS_SUMMARY_TABLE='<table class="table table-striped table-hover table-sm"><thead class="thead-light"><tr><th>Min</th><th>Max</th><th title="Max-Min">Range</th><th title="Arithmetic mean">Arithmetic mean</th><th title="weighted average">weighted mean</th><th>Mode</th><th title="Lower quartile">Q1</th><th title="Median">Q2</th><th title="Upper quartile">Q3</th><th title="Quartile difference">Qd</th><th>difference</th><th>Positive difference</th><th>Area</th><th>Points</th><th title="standard deviation">SD</th><th>Skewness</th><th>Kurtosis</th></tr></thead><tbody>';
...
function getTagSnapshotData(ajaxdata){
	...
	if(snap.Id>0){
		...
		alert('No matching variable name ['+TAG.FullName+'],please check!');
	}
}
//接收AJAX反馈的历史统计数据并解析
function getTagHistorySummary(ajaxdata){
	...
	HIS_SUMMARY = suma;
	HIS_SUMMARY_TABLE='<table class="table table-striped table-hover table-sm"><thead class="thead-light"><tr><th>Min</th><th>Max</th><th title="Max-Min">Range</th><th title="Arithmetic mean">Arithmetic mean</th><th title="Weighted average">Weighted mean</th><th>Mode</th><th title="Lower quartile">Q1</th><th title="Median">Q2</th><th title="Upper quartile">Q3</th><th title="Quartile difference">Qd</th><th title="Q1-1.5*Qd">Lower limit</th><th title="Q3+1.5*Qd">Upper limit</th><th>Difference</th><th>Positive difference</th><th>Area</th><th>Poings</th><th>Outlier points</th><th title="Standard deviation">SD</th></tr></thead><tbody>';
...
function getTagHistoryInterval(ajaxdata){//根据Ajax反馈的结果更新Tag的实时数据
	...
	HIS_INTERVAL_TABLE='<hr/><h3>Aggregate historical data at equal intervals</h1><br/><table class="table table-striped table-hover table-sm"><thead class="thead-light"><tr><th>Serial number</th><th>Time</th><th>Data</th></tr></thead><tbody>';
	if(dtarr != null){
	...
	$("#LoadDataMsg").html("The data loading is completed and takes:"+timediff/1000+"seconds");
}
//接收AJAX反馈的历史数据并解析
function getTagHistory(ajaxdata){//根据Ajax反馈的结果更新Tag的实时数据
	...
	HIS_TABLE='<hr/><h3>Original historical data</h1><br/><table class="table table-striped table-hover table-sm"><thead class="thead-light"><tr><th>Serial number</th><th>Time</th><th>Data</th></tr></thead><tbody>';
	if(dtarr != null){
		var hisdata=dtarr[TAG.FullName];
		for(var i=0;i< hisdata.length;i++){
...
{
	$("#LoadDataMsg").html("Loading snapshot data……");
	//调用公用的loadXMLDoc函数
...
{
	$("#LoadDataMsg").html("Loading statistics……");
	//调用公用的loadXMLDoc函数
    loadXMLDoc(urlstr,function()
    {
        if (xmlhttp.readyState==4 && xmlhttp.status==200)//请求处理完成，且状态OK
...
{
	$("#LoadDataMsg").html("Loading historical data……");
	//调用公用的loadXMLDoc函数
    loadXMLDoc(urlstr,function()
    {
        if (xmlhttp.readyState==4 && xmlhttp.status==200)//请求处理完成，且状态OK
...
{
	$("#LoadDataMsg").html("Loading equal interval historical data……");
	//调用公用的loadXMLDoc函数
    loadXMLDoc(urlstr,function()
    {
        if (xmlhttp.readyState==4 && xmlhttp.status==200)//请求处理完成，且状态OK
...
$(document).ready(function() {
	...
	$("#show_hisinterval_data").before("Aggregate data table")
	$("#show_his_data").before("Raw data table")
