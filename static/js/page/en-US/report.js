...
function getReportsDatas(datas){
	var htmlstr='<table class="table table-striped table-hover table-sm"><thead class="thead-light"><tr><th style="width:50px">Number</th><th style="width:50px">Name</th><th  style="width:50px">Parent level</th><th style="width:50px">Template</th><th style="width:50px">Debugg</th><th style="width:50px">State</th><th style="width:50px">Sort number</th><th style="width:100px">Calculate deadline</th><th style="width:100px">Update time</th><th style="width:100px">Operation</tr></thead><tbody>';
	if (datas!=null){
		var j=0
		for (var i=0;i<datas.length;i++){
			...
			if (node.Folder==0){
				...
				j++;
				var btedit='<button class="btn btn-outline-primary btn-sm" onclick="requestTplLists('+node.Id+');">View results</button>';
...
function getReportLists(datas){
	...
	*/
	...
	<table class="table table-striped table-hover table-sm"><thead class="thead-light"><tr><th style="width:50px">Number </th><th style="width:200px">Report time</th><th style="width:200px">Generation / edit time</th><th>File size</th><th>Operation</th></tr></thead><tbody>`;
	if (datas!=null){
		datas = bubbleSort(datas);
		for (var i=0;i<datas.length;i++){
			var node=datas[i];
			var fname = node.FileName.split(".",1);
			var btview=`<button class="btn btn-outline-primary btn-sm" id="`+fname+`" onclick="onTplView('`+node.FileName+`',this.id);" title="Preview files online">Preview</button>`;
			var btdown=`<button class="btn btn-outline-success btn-sm" onclick="onTplDown('`+node.FileName+`');" title="Download report file to local">Download</button>`;

			if(i==0){
				onTplView(node.FileName);
				LAST_SELECTED_FILE = fname;
				btview=`<button class="btn btn-primary btn-sm" id="`+fname+`" onclick="onTplView('`+node.FileName+`',this.id);" title="Preview files online">Preview</button>`;
			}
...
$(document).ready(function() {
	...
	$('#OperationMsg').html('<strong id = "works">Note：</strong> The online preview format is not exactly the same as the actual Excel file format. Please refer to the downloaded Excel file!At present, the online preview only has the ability to calculate simple Excel formulas. The calculation results of using Excel formulas are for reference only. Please refer to the results in the Downloaded Excel file.');

	$('#CellAxis').attr("title","Select the coordinates of the cell");
	$('#selectTdValue').attr("title","Select the value or formula of the cell");
