...
function getCfg(ajaxdata){
	...
	*/
	...
	var htmlstr='<div class="col-12 border-bottom"><h5>KPI configuration information</h5></div>';
	NODES_CFG = node;
		
	htmlstr+='<div class="col-sm-3 form-inline"><strong>ID:</strong>'+node.Id+'</div>';
	htmlstr+='<div class="col-sm-3 form-inline"><strong>Distributed ID:</strong>'+node.DistributedId+'</div>';
	htmlstr+='<div class="col-sm-3 form-inline"><strong>Main label type:</strong>'+node.TagType+'</div>';
	htmlstr+='<div class="col-sm-3 form-inline"><strong>Master tag ID:</strong>'+node.TagId+'</div>';
	htmlstr+='<div class="col-sm-12 form-inline"><strong>KPI label:</strong>'+node.KpiTag+'</div>';
	htmlstr+='<div class="col-sm-12 form-inline"><strong>KPI name:</strong>'+node.KpiName+'</div>';
	if(node.CalcKpiIndexDic.Id>0){
		htmlstr+='<div class="col-sm-6 form-inline"><strong>Calculation script:</strong><div style="width:60%"><form><textarea class="form-control" style="width:100%" rows="1" disabled>'+node.CalcKpiIndexDic.Script+'</textarea></form></div></div>';
		htmlstr+='<div class="col-sm-6 form-inline"><strong>Script description:</strong>'+node.CalcKpiIndexDic.KpiNameCn+'</div>';
	}else{
		htmlstr+='<div class="col-sm-12 form-inline"><strong>Calculation script:</strong><div style="width:100%"><form><textarea class="form-control" style="width:100%" disabled>'+node.Script+'</textarea></form></div></div>';
	}
	htmlstr+='<div class="col-sm-3 form-inline"><strong>Unit of measurement:</strong>'+node.CfgUnit+'</div>';
	htmlstr+='<div class="col-sm-3 form-inline"><strong>Remark:</strong>'+node.Description+'</div>';
	htmlstr+='<div class="col-sm-3 form-inline"><strong>Calculation period:</strong>'+getPeriodStr(node.Period)+'</div>';
	htmlstr+='<div class="col-sm-3 form-inline"><strong>Calculate offset time(min):</strong>'+node.OffsetMinutes+'</div>';
	htmlstr+='<div class="col-sm-3 form-inline"><strong>Start time:</strong>'+node.StartTime+'</div>';
	htmlstr+='<div class="col-sm-3 form-inline"><strong>Latest data time:</strong>'+node.LastCalcTime+'</div>';
	htmlstr+='<div class="col-sm-3 form-inline"><strong>Last calculation time:</strong>'+node.UpdateTime+'</div>';
	htmlstr+='<div class="col-sm-3 form-inline"><strong>Creation time:</strong>'+node.CreateTime+'</div>';
	$("#CfgMsg").html(htmlstr);
}
...
function getTagTypeList(ajaxdata){
	...
	*/
	...
	var htmlstr='<div class="col-12 border-bottom"><h5>KPI Statistical</h5></div>';
	htmlstr+='<div class="col-12"><table class="table table-striped table-hover table-sm"><tbody><tr><th>Serial number</th><th>Main variable type</th><th>Primary variable type name</th><th>Number of indicators</th></tr>';
	var kpicnt=0;
	...
	htmlstr+='<tr>';
	htmlstr+='<th colspan="3">total</th>';
	htmlstr+='<th>'+kpicnt+'</th>';
	htmlstr+='</tr>';
	htmlstr+='</tbody></table></div>';
	$("#CfgMsg").html(htmlstr);
	refreshEcharts_Pie('KPI statistics');
}
...
function getKpiTagTypePeriodInfo(ajaxdata){
	...
	*/
	...
	PIE_DATA.splice(0,PIE_DATA.length);//清空数组
	PIE_LEGEND.splice(0,PIE_LEGEND.length);//清空数组
	var htmlstr='<div class="col-12 border-bottom"><h5>KPI statistics</h5></div>';
	htmlstr+='<div class="col-12"><table class="table table-striped table-hover table-sm"><tbody><tr><th>Serial number</th><th>Cycle type</th><th>Number of indicators</th></tr>';
	var kpicnt=0;
	for(var i=0;i<nodes.length;i++){
		var node=nodes[i];
		htmlstr+='<tr>';
...
function getPeriodStr(period){
	...
	switch(parseInt(period)){
		case -1:
			periodstr='Every hour';
			break;
		case -2:
			periodstr='Every shift';
			break;
		case -3:
			periodstr='Everyday';
			break;
...
function getDatas(ajaxdata){
	...
	*/
	...
	DATAS = datas;
	var htmlstr='<table class="table table-striped table-hover table-sm"><tr colspan="5"><h5>KPI Calculation result data</h5></tr><thead class="thead-light">';
	htmlstr+='<tr><th>Serial number</th><th>Data time </th><th>Calculation time</th><th>Value </th><th>Unit</th></tr></thead><tbody>';
	...
	htmlstr+='<strong>Min:</strong>'+DataToFixed(min,"float",dot)+'&nbsp;';
	htmlstr+='<strong>Max:</strong>'+DataToFixed(max,"float",dot)+'&nbsp;';
	htmlstr+='<strong>Avg:</strong>'+avg+'&nbsp;';
	htmlstr+='<strong>Sum:</strong>'+DataToFixed(kpisum,"float",dot);
