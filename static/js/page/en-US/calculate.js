...
function getURL() {
    ...
    for (var i = 0; i < timeArr.length ; i++){
        if (i == 0){
            url.push("http://192.168.3.39:8080/api/script?script=tag("+inputState[2]+").fc("+inputState[3]+")&beginTime="+inputState[0]+":00&endTime="+timeArr[i]+":00&baseTime=2019-12-17 08:00:00&shifthours=8")
        }else {
            url.push("http://192.168.3.39:8080/api/script?script=tag("+inputState[2]+").fc("+inputState[3]+")&beginTime="+inputState[0]+":00&endTime="+timeArr[i]+"&baseTime=2019-12-17 08:00:00&shifthours=8")
        }
        // console.log(url[i])
    }
    return url
...
$(document).ready(function (){
    ...
    $("#BeginTimes").html("BeginTime：")
    $("#EndTimes").html("EndTime：")
    $("#tag_id").html("Tag_id： &nbsp;&nbsp;")
    $("#fc").html("Calculation method： ")
    $("#Total").html("Cycle consumption time")
    $("#plusDiff").html("Periodic ore treatment")
    $("#search").html("Search")
    $("#save").html("Save to database")
    $("#cj1").html("Workshop： ")
    $("#sb1").html("Equipment：")
    $("#bl1").html("Variable：")
    $("#HisDataTable").html('<strong>Operation：</strong> Please enter the ID and time range of a label in the input box above to display the data！');
})