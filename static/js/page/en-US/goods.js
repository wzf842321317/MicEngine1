...
{	
	$("#DataMsg").html('<div class="alert alert-warning">Loading equal interval historical data……</div>');
	//调用公用的loadXMLDoc函数
    loadXMLDoc(urlstr,function()
    {
        if (xmlhttp.readyState==4 && xmlhttp.status==200)//请求处理完成，且状态OK
...
function getGoodsCfg(ajaxdata){
	...
	var htmlstr='<div class="col-12 border-bottom"><h5>Basic material information</h5></div>';
	for(var i=0;i<nodes.length;i++){
		var node=nodes[i];
		NODES_CFG = node;
		var danger = '<span class="badge badge-success">NO</span>';;
		if(node.Goods.GoodsIsDanger==1){
			danger = '<span class="badge badge-danger">Yes</span>';
		}
		htmlstr+='<div class="col-sm-4 form-inline"><strong>Name:</strong>'+node.Goods.GoodsName+'</div>';
		htmlstr+='<div class="col-sm-4 form-inline"><strong>Workshop:</strong>'+node.Workshop.WorkshopName+'</div>';
		htmlstr+='<div class="col-sm-4 form-inline"><strong>Level:</strong>'+LEVEL_NAMES[node.TreeLevelCode]+'</div>';
		htmlstr+='<div class="col-sm-4 form-inline"><strong>Administrative department:</strong>'+node.Goods.DeptManage.DepartmentName+'</div>';
		htmlstr+='<div class="col-sm-4 form-inline"><strong>User code:</strong>'+node.Goods.UserNameCode+'</div>';
		htmlstr+='<div class="col-sm-4 form-inline"><strong>Specification and model:</strong>'+node.Goods.Specifications+'</div>';
		htmlstr+='<div class="col-sm-4 form-inline"><strong>Unit of measurement:</strong>'+node.Goods.GoodsConsumeUnit+'</div>';
		htmlstr+='<div class="col-sm-4 form-inline"><strong>Packing method:</strong>'+node.Goods.PackingMode+'</div>';
		htmlstr+='<div class="col-sm-4 form-inline"><strong>Packaging unit:</strong>'+node.Goods.PackingUnit+'</div>';
		htmlstr+='<div class="col-sm-4 form-inline"><strong>Package specification:</strong>'+node.Goods.PackingModel+'</div>';
		htmlstr+='<div class="col-sm-4 form-inline"><strong>Storage conditions:</strong>'+node.Goods.PreserveCondition+'</div>';
		htmlstr+='<div class="col-sm-4 form-inline"><strong>Transport conditions:</strong>'+node.Goods.TransportCondition+'</div>';
		htmlstr+='<div class="col-sm-4 form-inline"><strong>Type of shipping:</strong>'+node.Goods.TransportMode+'</div>';
		htmlstr+='<div class="col-sm-4 form-inline"><strong>Dangerous goods:</strong>'+danger+'</div>';
		htmlstr+='<div class="col-sm-4 form-inline"><strong>Factory number:</strong>'+node.Goods.FactorySerialNum+'</div>';
		htmlstr+='<div class="col-sm-4 form-inline"><strong>Manufacturer:</strong>'+node.Goods.ManufacturerName+'</div>';
		htmlstr+='<div class="col-sm-4 form-inline"><strong>Batch number:</strong>'+node.Goods.ProductionBatchNum+'</div>';
		htmlstr+='<div class="col-sm-4 form-inline"><strong>Reference unit price:</strong>'+node.Goods.ConsumeUnitPrice+'</div>';
		htmlstr+='<div class="col-sm-4 form-inline"><strong>Conversion ratio:</strong>'+node.Goods.ScalingFactor+'</div>';
		htmlstr+='<div class="col-sm-4 form-inline"><strong>Total consumption in selected time range:</strong><span id="SumData"></span></div>';
	}
	$("#CfgMsg").html(htmlstr);
}
...
function getGoodsDatas(ajaxdata){
	...
	if(SELECT_LEAF==true){//选择的是物耗标签节点
		htmlstr+='<tr><th>Number</th><th>Start time</th><th>End time</th><th>Interval(h)</th><th>Amount</th><th>Company</th><th>Remark</th><th>Creater</th><th>Creation time</th><th>Editor</th><th>Editing time</th></tr></thead><tbody>';
		var dsum=0;
		...
		htmlstr+='<tr><th colspan="4">Total</th><th>'+DataToFixed(dsum,"float",2)+'</th><th colspan="6">'+unit+'</th><tr>';
		$("#SumData").text(DataToFixed(dsum,"float",2)+unit);
	}else{//当前选择的是工厂层级节点
		var goodsnames=new Array();
		htmlstr+='<tr><th>Number</th><th>Material name</th><th>Specification and model</th><th>Workshop</th><th>Level</th><th>Start time</th><th>End time</th><th>Interval(h)</th><th>Amount</th><th>Company</th><th>Remark</th></tr></thead><tbody>';
		for(var i=0;i<datas.length;i++){
			data=datas[i];
		...
		//按名称、型号过滤
		var fillter='<div class="col-12 border-bottom"><h5>Material screening</h5></div>';
		fillter+='<div class="col form-inline"><label class="font-weight-bold" for="GoodsName">Material name:</label><select class="form-control" id="GoodsName" onchange="onFillterByName(this.options[this.options.selectedIndex].value)" ><option value="0">All</option>';
		for (let k in goodsnames) {//名称过滤选择框
			fillter+='<option value="'+k+'">'+k+'</option>';
		}
		fillter+='</select></div>';
		fillter+='<div class="col form-inline" id="TypeFillter"><label class="font-weight-bold" for="GoodsType">Specification and model:</label><select class="form-control" id="GoodsType" onchange="onFillterByType(this.options[this.options.selectedIndex].value)" disabled="disabled"><option value="0">All</option></select></div>';
		fillter+='<div class="col form-inline" id="GoodsSumDiv" style="display:none"><label class="font-weight-bold" for="GoodsSum">Total:</label><span id="GoodsSum"></span></div>';//显示累积
		$("#CfgMsg").html(fillter);
	}
	htmlstr+='</tbody></table>';
...
function onFillterByName(name){
	...
	if(name !='0'){//不是全部的时候才显示累积,更新型号筛选
		...
		$("#TypeFillter").html("");
		var fillter='<label class="font-weight-bold" for="GoodsType">Specification and model:</label><select class="form-control" id="GoodsType" onchange="onFillterByType(this.options[this.options.selectedIndex].value)"><option value="0">All</option>';
		...
		$("#TypeFillter").html("");
		var fillter='<label class="font-weight-bold" for="GoodsType">Specification and model:</label><select class="form-control" id="GoodsType" onchange="onFillterByType(this.options[this.options.selectedIndex].value)" disabled="disabled"><option value="0">All</option>';
		fillter+='</select>';
		$("#TypeFillter").html(fillter);
		$("#GoodsSumDiv").hide();
	}
...
function onFillterByType(type){
	var htmlstr='<table class="table table-striped table-hover table-sm"><tr colspan="5"><h5>Material consumption data</h5></tr><thead class="thead-light">';
	htmlstr+='<tr><th>Number</th><th>Material name</th><th>Specification and model</th><th>Workshop</th><th>Level</th><th>Strat time</th><th>End time</th><th>Interval(h)</th><th>Amount</th><th>Company</th><th>Remark</th></tr></thead><tbody>';
...
$(document).ready(function() {
	...
	$('#OperationMsg').html('<strong id = "works">handle：</strong> Please select a sample node in the left structure tree to display the data!');
