...
function showSampleMsg(samp){
	var htmlstr='<div class="col-12 border-bottom"><h5>Sample basic information</h5></div>';
	htmlstr+='<div class="col-sm-3 form-inline"><strong>Sample name:</strong>'+samp.Name+'</div>';
	htmlstr+='<div class="col-sm-3 form-inline"><strong>Sampling location:</strong>'+samp.SamplingSite+'</div>';
	htmlstr+='<div class="col-sm-3 form-inline"><strong>Sample type:</strong>'+samp.FuncName+'</div>';
	if(samp.IsRegular==1){
		reg='<span class="badge badge-success">Yes</span>';
	}else{
		reg='<span class="badge badge-danger">No</span>';
	}
	htmlstr+='<div class="col-sm-3 form-inline"><strong>Regular pattern:</strong>'+reg+'</div>';
...
function onShowPic(picurl){
	...
	ShowModal("Sample photos",htmlstr);
}
...
function getSampleLabResult(ajaxdata){
	...
	var htmlstr='<table class="table table-striped table-hover table-sm"><tr colspan="5"><h5>Laboratory result information</h5></tr><thead class="thead-light"><tr><th>Serial number</th><th>Sampling time</th><th>Sample delivery time</th><th>Sample number</th><th>Sample status</th><th>Index</th><th>key word</th><th>Result</th><th>Company</th><th>Sample photos</th></tr></thead><tbody>';
	for(var i=0;i<SAMPLE_LAB_RESULT.length;i++){
		...
		case 0:
			labtype='<span class="badge badge-secondary">Not received</span>';
			break;
		case 1:
			labtype='<span class="badge badge-info">Not tested</span>';
			break;
		case 2:
			labtype='<span class="badge badge-warning">Not reviewed</span>';
			break;
		case 3:
			labtype='<span class="badge badge-success">Reviewed</span>';
			break;
		default:
			break;
		}
		var picbtn='<button type="button" class="btn btn-outline-primary btn-sm" onclick="onShowPic(this.id)" id="'+sampPicPath+samp.SampleToLab.SampleSitePhoto+'">Display</button>';
