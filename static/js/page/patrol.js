...
function showCheckSiteMsg(){
	var htmlstr='<div class="col-12 border-bottom"><h5>巡检点信息</h5></div>';
	htmlstr+='<div class="col form-inline"><strong>设备点ID:</strong>'+SELECT_NODE.Id+'</div>';
	htmlstr+='<div class="col form-inline"><strong>巡检点ID:</strong>'+SELECT_NODE.SiteId+'</div>';
	htmlstr+='<div class="col form-inline"><strong>巡检点名:</strong>'+SELECT_NODE.Name+'</div>';
	htmlstr+='<div class="col form-inline"><strong>所属线路:</strong>'+SELECT_NODE.LineName+'</div>';
	htmlstr+='<div class="col form-inline"><strong>所属部门:</strong>'+SELECT_NODE.DeptName+'</div>';
...
{	
    ...
    loadXMLDoc(urlstr,function()
    {
		...
		{
			ShowModal('错误','<div class="alert alert-danger">'+decodeURI(xmlhttp.responseText)+'</div>');
		}
    });
}
...
function getCheckSiteExePicUrl(ajaxdata){
	...
	ShowModal("巡检照片",htmlstr);
}
//解析配置信息
function getPatrolCfg(ajaxdata){
	var nodes = eval("("+ajaxdata+")"); 
	NODES_CFG = nodes;
	var htmlstr='<div class="col-12 border-bottom"><h5>检查项信息</h5></div><table class="table table-striped table-hover table-sm"><thead class="thead-light">';
	htmlstr+='<tr><th>序号</th><th>检查项</th><th>检查方法</th><th>检查内容</th><th>单位</th><th>最小值</th><th>最大值</th><th>下下限</th><th>下限</th><th>上限</th><th>上上限</th><th>趋势</th><th>统计</th></tr></thead><tbody>';
	for(var i=0;i<nodes.length;i++){
		var data=nodes[i];

		//对于有策略上限的检查项(数字类型)，显示趋势按钮，否则禁用
		var trenthbtn=data.MeasMaxvalue==0?('<button type="button" class="btn btn-outline-secondary btn-sm" onclick="onShowTrenth(this.name,this.id)" id="'+data.Id+'" disabled="disabled">趋势</button>'):('<button type="button" class="btn btn-outline-primary btn-sm" onclick="onShowTrenth(this.name,this.id)" id="'+data.Id+'" name="'+data.CheckName+'">趋势</button>');
		//对于有策略上限的检查项(数字类型)，显示分析按钮，否则禁用
		var analysebtn=data.MeasMaxvalue==0?('<button type="button" class="btn btn-outline-secondary btn-sm" onclick="onShowAnalyse(this.name,this.id)" id="'+data.Id+'" disabled="disabled">统计</button>'):('<button type="button" class="btn btn-outline-primary btn-sm" onclick="onShowAnalyse(this.name,this.id)" id="'+data.Id+'" name="'+data.CheckName+'">统计</button>');
...
function getCheckSiteMsg(ajaxdata){
	...
	var htmlstr='<div class="col-12 border-bottom"><h5>巡检站点信息</h5></div><table class="table table-striped table-hover table-sm"><thead class="thead-light">';
	htmlstr+='<tr><th>序号</th><th>检查点ID</th><th>检查点名</th><th>站点ID</th><th>站点名</th><th>所属巡检线路</th><th>所属部门</th><th>计划检查项(项)</th><th>实际检查项(项)</th><th>完成率(%)</th></tr></thead><tbody>';
	if(nodes!=null){
		for(var i=0;i<nodes.length;i++){
			var data=nodes[i];
	...
	htmlstr='<div class="col-12 border-bottom"><h5>检查点统计信息</h5></div>';
	htmlstr+='<div class="col form-inline"><strong>检查点数量:</strong>'+(nodes==null?0:nodes.length)+'个</div>';
	htmlstr+='<div class="col form-inline"><strong>站点数量:</strong>'+siteCnt+'个</div>';
	htmlstr+='<div class="col form-inline"><strong>巡检线路数量:</strong>'+lineCnt+'个</div>';
	htmlstr+='<div class="col form-inline"><strong>管理部门数量:</strong>'+deptCnt+'个</div>';
	htmlstr+='<button type="button" class="btn btn-outline-primary btn-sm" onclick="onShowRelation()">查看图谱</button>'
	$("#CheckSiteMsg").html(htmlstr);
...
function getLeafsDatas(datas){
	var htmlstr='<div class="col-12 border-bottom"><h5>检查结果</h5></div><table class="table table-striped table-hover table-sm"><thead class="thead-light">';
	if(SELECT_LEAF==true){//选择的是巡检标签节点
		var checkcnt=0;//已经检查项数量
		var yichangcnt=0;//异常项数量
		htmlstr+='<tr><th>序号</th><th>检查项</th><th>开始时间</th><th>结束时间</th><th>实际检查时间</th><th>值</th><th>单位</th><th>检查状态</th><th>值状态</th><th>异常</th><th>备注</th><th>检查人</th><th>照片</th></tr></thead><tbody>';
		for(var i=0;i<datas.length;i++){
			data=datas[i];
			checkcnt += data.CheckStatus;//检查项累加
			yichangcnt += data.Abnormal;//异常项累加
			if(data.CheckStatus==0){
				if (PERSON_GROUP.hasOwnProperty("未检")){//如果存在
					PERSON_GROUP["未检"]+=1;//加一
				}else{//如果不存在
					PERSON_GROUP["未检"]=1;//赋值
				}
			}else{//已检
			...
			var checkstatus=data.CheckStatus==1?'<span class="badge badge-success">已检</span>':'<span class="badge badge-warning">未检</span>';
			var abnormal=data.Abnormal==1?'<span class="badge badge-success">否</span>':'<span class="badge badge-danger">是</span>';

			var alarmtype='';
			switch(data.AlarmType){
			case 1:
				alarmtype='<span class="badge badge-danger">低于下下限</span>';
				break;
			case 2:
				alarmtype='<span class="badge badge-warning">低于下限</span>';
				break;
			case 3:
				alarmtype='<span class="badge badge-success">正常</span>';
				break;
			case 4:
				alarmtype='<span class="badge badge-warning">高于上限</span>';
				break;
			case 5:
				alarmtype='<span class="badge badge-danger">高于上上限</span>';
				break;
			case 6:
				alarmtype='<span class="badge badge-dark">超量程</span>';
				break;
			default:
				break;
			}
	...
	var tjhtml='<div class="col-12 border-bottom"><h5>检查项统计</h5></div>';
	tjhtml+='<div class="col form-inline"><strong>已检项:</strong>'+checkcnt+'条</div>';
	tjhtml+='<div class="col form-inline"><strong>未检项:</strong>'+(i-checkcnt)+'条</div>';
	tjhtml+='<div class="col form-inline"><strong>检验率:</strong>'+DataToFixed((i>0?checkcnt/i*100:0),"float",2)+'%</div>';
	tjhtml+='<div class="col form-inline"><strong>异常项:</strong>'+(i-yichangcnt)+'条</div>';
	tjhtml+='<div class="col form-inline"><strong>异常率:</strong>'+DataToFixed((i>0?(i-yichangcnt)/i*100:0),"float",2)+'%</div>';
	tjhtml+='<div class="col form-inline"><strong>仅显示异常项:</strong><input id="checkbox" type="checkbox" class="form-check-input" '+(ONLY_SHOW_ABNORMAL==1?"checked":"")+' onclick="onlyshowabnomal()"/></div>';
	tjhtml+='<div class="col form-inline"><button type="button" class="btn btn-outline-primary btn-sm" onclick="onShowEcharts()">按人统计</button></div>';
	$("#TongJi").show();
	$("#TongJi").html(tjhtml);
}
//解析节点数据
function getNodesDatas(datas){
	var htmlstr='<div class="col-12 border-bottom"><h5>检查结果</h5></div><table class="table table-striped table-hover table-sm"><thead class="thead-light">';
	if(SELECT_LEAF==true){//选择的是巡检标签节点
		htmlstr+='<tr><th>序号</th><th>检查项</th><th>检查状态</th><th>开始时间</th><th>结束时间</th><th>实际检查时间</th><th>值</th><th>单位</th><th>异常</th><th>备注</th><th>检查人</th></tr></thead><tbody>';
		for(var i=0;i<datas.length;i++){
			data=datas[i];
...
*/
	var items=["最小值","最大值","极差","标准差","平均数","众数","中位数","下四分位数(Q1)","上四分位数(Q3)","四分位差(IQR)","离群下限","离群上限","离群点数","数据点数"];
	...
	if(stat_datas.length>2){
		htmlstr+='<tr><th rowspan="2">统计项目</th><th colspan="2">原始数据的统计数据</th><th colspan="2">用中位数替换离群点后的统计数据</th></tr>'
		htmlstr+='<tr><th>原始数据</th><th>增量数据</th><th>原始数据</th><th>增量数据</th></tr></thead><tbody>'
	}else{
		htmlstr+='<tr><th>统计项目</th><th>原始数据</th><th>增量数据</th></tr></thead><tbody>'
	}
	for(let i in items){
		htmlstr+='<tr>';
	...
	htmlstr+='</tbody></table>';
	ShowModal(tagname+"统计数据",htmlstr);
}
...
function onFillterByName(name){
	...
	if(name !='0'){//不是全部的时候才显示累积,更新型号筛选
		...
		$("#TypeFillter").html("");
		var fillter='<label class="font-weight-bold" for="PatrolType">规格型号:</label><select class="form-control" id="PatrolType" onchange="onFillterByType(this.options[this.options.selectedIndex].value)"><option value="0">全部</option>';
		...
		$("#TypeFillter").html("");
		var fillter='<label class="font-weight-bold" for="PatrolType">规格型号:</label><select class="form-control" id="PatrolType" onchange="onFillterByType(this.options[this.options.selectedIndex].value)" disabled="disabled"><option value="0">全部</option>';
		fillter+='</select>';
		$("#TypeFillter").html(fillter);
		$("#PatrolSumDiv").hide();
	}
...
$(document).ready(function() {
	...
	$('#OperationMsg').html('<strong >操作：</strong>  请在左侧结构树中选择一个样本节点以便显示数据！');
});
