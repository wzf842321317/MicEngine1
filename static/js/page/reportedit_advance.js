    ...
    xhr.onload=function(){
        if(xhr.status==200){
			if(xhr.responseText!="ok"){
                ...
                setCellStatus(LAST_SELECTED_CELL,true);
                ShowModal("编译错误",'<div class="alert alert-danger"><strong>错误:</strong><br>'+xhr.responseText+'</div>');
            }else{
                CELLSMAP[LAST_SELECTED_CELL].Status=1;
                setCellStatus(LAST_SELECTED_CELL,true);
                ShowModal("编译通过",'<div class="alert alert-success"><strong>成功:</strong>编译通过！</div>');
                onPostSetCellValue(LAST_SELECTED_CELL,cellinput);//保存单元格的值
            }
        }
    }
    ...
    xhr.onload=function(){
        if(xhr.status==200){
            if(xhr.responseText!="ok"){
                ...
                ShowModal("保存文件错误",'<div class="alert alert-danger"><strong>错误:</strong><br>'+xhr.responseText+'</div>');
            }else{
